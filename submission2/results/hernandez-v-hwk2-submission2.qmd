...
**Methods:**
1. **INV**: Nearest neighbor matching with inverse variance distance
2. **MAH**: Nearest neighbor matching with Mahalanobis distance  
3. **IPW**: Inverse propensity score weighting
4. **OLS**: Linear regression with interactions

All methods control for FFS cost quartiles (Q2, Q3, Q4 dummy variables).
```{python}
import pandas as pd
import os

# Load pre-computed results from analysis
base_path = os.path.expanduser("~/Documents/econ470")
table3 = pd.read_csv(os.path.join(base_path, "data", "output", "table3_question7.csv"))

print("TABLE 3: Average Treatment Effects")
print("High HHI vs. Low HHI Markets (2018)")
print("="*70)
...
mean_ate = table3['ATE'].mean()

print(f"\nConsistency Metrics:")
print(f"  Mean ATE: ${mean_ate:.2f}")
print(f"  Range: ${max_diff:.2f}")
print(f"  Standard Deviation: ${std_ate:.2f}")

if max_diff < 0.50:
    print("\n✓ All estimates are essentially identical (as expected)")
else:
    print("\n⚠ WARNING: Estimates differ substantially")
    print("   This indicates potential data or implementation issues")
```
...
| Estimator | ATE |
|-----------|---------|
| INV (Inverse Variance Matching) | $15.95 |
| MAH (Mahalanobis Matching) | $24.36 |
| IPW (Inverse Propensity Weighting) | $19.61 |
| OLS (Linear Regression) | $19.61 |

**Interpretation:**

Taking the **mean across all four methods** ($19.88), the evidence suggests that plans in high HHI (uncompetitive) markets bid approximately **$20 more** than plans in low HHI (competitive) markets, after controlling for FFS cost quartiles.
...
# Load Question 7 results
base_path = os.path.expanduser("~/Documents/econ470")
ate_results = pd.read_csv(os.path.join(base_path, "data", "output", "table3_question7.csv"))

# Calculate similarity metrics
ate_mean = ate_results['ATE'].mean()
ate_std = ate_results['ATE'].std()
ate_cv = (ate_std / abs(ate_mean)) * 100 if ate_mean != 0 else 0
...
The results show **substantial variation** across the four estimation methods; however, the results are supposed to be similar across the estimators which can only be seen in the OLS and IPW estimators.
...
For this analysis, I selected OLS regression as my preferred estimator.
